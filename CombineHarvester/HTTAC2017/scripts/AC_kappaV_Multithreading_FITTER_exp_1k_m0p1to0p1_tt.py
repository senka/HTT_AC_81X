...
points = 1089 # 2000 gives very nice contours
cvMax = 2.0
...
def runFit(points, toys, j) :
    ...
    toRun.append('-n')
    toRun.append('CvAC_FITTER_exp_m0p1to0p1_1k_m0p1to0p1_tt.%i' % j)
    toRun.append('-M')
    ...
    toRun.append('muV=0.0,%s:muf=0.0,10.0:CMS_zz4l_fai1=-0.1,0.1' % cvMax)
    toRun.append('tt/125/fa03_Interference_Workspace_tt.root')
    toRun.append('--algo=grid')
    toRun.append('--points=%i' % points)

    toRun.append('--setParameters')
    toRun.append('muV=1.,CMS_zz4l_fai1=0.,muf=1.') 
    #toRun.append('-t')
    #toRun.append('-1')
    #toRun.append('--expectSignal=1')
