...
sigma_SM_HTT=968.674
sigma_3_HTT=10909.54

sigma_SM_HZZ=290.58626
sigma_3_HZZ=44.670158
sigma_13_int=1937.15
sigma_13_intOnly=(sigma_13_int-2.*sigma_SM_HTT)*math.sqrt(sigma_3_HTT/sigma_SM_HTT)
...
#    limit, "muV", "CMS_zz4l_fai1", '2*deltaNLL', 'quantileExpected > -0.5')
    limit, "muV*( (1.-abs(CMS_zz4l_fai1))*%s + abs(CMS_zz4l_fai1)*%s*%s/%s + sign(CMS_zz4l_fai1)*sqrt((1-abs(CMS_zz4l_fai1))*abs(CMS_zz4l_fai1)*%s/%s)*%s )/(%s)" % (sigma_SM_HTT,sigma_SM_HZZ,sigma_3_HTT,sigma_3_HZZ,sigma_SM_HTT,sigma_3_HTT,sigma_13_intOnly,sigma_SM_HTT), "CMS_zz4l_fai1", '2*deltaNLL', 'quantileExpected > -0.5')
best = plot.TGraphFromTree(
#    limit, "muV", "CMS_zz4l_fai1", 'deltaNLL == 0')
    limit, "muV*( (1.-abs(CMS_zz4l_fai1))*%s + abs(CMS_zz4l_fai1)*%s*%s/%s + sign(CMS_zz4l_fai1)*sqrt((1-abs(CMS_zz4l_fai1))*abs(CMS_zz4l_fai1)*%s/%s)*%s )/(%s)" % (sigma_SM_HTT,sigma_SM_HZZ,sigma_3_HTT,sigma_3_HZZ,sigma_SM_HTT,sigma_3_HTT,sigma_13_intOnly,sigma_SM_HTT), "CMS_zz4l_fai1", 'deltaNLL == 0')
...
#        limit_bg, "muV", "CMS_zz4l_fai1", 'deltaNLL == 0')
        limit_bg, "muV*( (1.-abs(CMS_zz4l_fai1))*%s + abs(CMS_zz4l_fai1)*%s*%s/%s + sign(CMS_zz4l_fai1)*sqrt((1-abs(CMS_zz4l_fai1))*abs(CMS_zz4l_fai1)*%s/%s)*%s )/(%s)" % (sigma_SM_HTT,sigma_SM_HZZ,sigma_3_HTT,sigma_3_HZZ,sigma_SM_HTT,sigma_3_HTT,sigma_13_intOnly,sigma_SM_HTT), "CMS_zz4l_fai1", 'deltaNLL == 0')
...
axis = ROOT.TH2D(hists.GetName(),hists.GetName(),hists.GetXaxis().GetNbins(),0,hists.GetXaxis().GetXmax(),hists.GetYaxis().GetNbins(),hists.GetYaxis().GetXmin(),hists.GetYaxis().GetXmax())

axis.Reset()
axis.GetXaxis().SetTitle(args.x_title)
axis.GetXaxis().SetLabelSize(0.025)
axis.GetYaxis().SetLabelSize(0.025)
axis.GetYaxis().SetTitle("f_{a3}cos(#phi_{#lower[-0.25]{a3}})")
...
if args.sm_exp:
    ...
    best_sm.Draw("P SAME")
    legend.AddEntry(best_sm, "Expected: SM H(125)", "P")
if args.bg_exp:
...
canv.Print('_muVSM.pdf')
canv.Print('_muVSM.png')
canv.Print('_muVSM.C')
canv.Close()
